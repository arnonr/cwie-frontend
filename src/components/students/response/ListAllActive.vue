      ...
      <tbody v-if="items.length != 0">
        <tr v-for="(it, idx) in items" :key="idx">
          <td class="text-center">
            {{ convertDate(it.response_send_at) }}
          </td>
          ...
          <td>{{ convertDate(it.confirm_response_at) }}</td>
          <td class="text-center">
            <span
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const headerColumn = [
      ...
      { column_name: "response_result", title: "ผลการตอบกลับ", sort: true },
      {
        column_name: "response_document_file",
        title: "ไฟล์หนังสือผลการตอบกลับ",
        sort: true,
      },
      {
        column_name: "confirm_response_at",
        title: "วันที่รับทราบหนังสือ",
        sort: true,
      },
