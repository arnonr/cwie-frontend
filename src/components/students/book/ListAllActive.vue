      ...
      <tbody v-if="items.length != 0">
        <tr v-for="(it, idx) in items" :key="idx">
          <td class="text-center">
            <input
              :value="it.id"
              v-if="it.form_status_id > 5"
              class="form-check-input chkItem"
              type="checkbox"
              id="flexCheckDefault"
              @change="toggleSelectedItem(it.id)"
              :checked="selectedItem.includes(it.id)"
            />
          </td>
          <td class="text-center">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const toggleSelectedItem = (itemId: number) => {
      if (selectedItem.value.includes(itemId)) {
        emit(
          "update:selectedItem",
          selectedItem.value.filter((id) => id !== itemId)
        );
      } else {
        emit("update:selectedItem", [...selectedItem.value, itemId]);
      }
    };
